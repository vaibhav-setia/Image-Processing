...
int main(int argc, char *argv[]) {
	...
	int noise_stddev=1;
	string input_filename="002.png", output_filename="write.png";   // Have a blurred image here
	...
	Mat raw_sample = imread("001.png", CV_LOAD_IMAGE_GRAYSCALE); // READ THE SAMPLE IMAGE WHOSE SPECTRUM IS EXPECTED TO BE SIMILAR TO THE IMAGE
...
//clas---------------------------------
Mat cls = (Mat_<double>(3,3) << 0,-1,0,-1,4,-1  ,0,-1,0);
int w = padded.size().width-cls.cols;
	int h = padded.size().height-cls.rows;

	int r = w/2;
	int l = padded.size().width-cls.cols -r;

	int b = h/2;
	int t = padded.size().height-cls.rows -b;
copyMakeBorder(cls,cls,t,b,l,r,BORDER_CONSTANT,Scalar::all(0));
Mat cls_planes[2];
shift(cls);
Mat clsplane[] = {Mat_<float>(cls), Mat::zeros(cls.size(), CV_32F)};
Mat clscomplex;
    merge(clsplane, 2, clscomplex); 
dft(clscomplex, clscomplex);
split(clscomplex,clsplane);
magnitude(clsplane[0], clsplane[1], clsplane[0]);// planes[0] = magnitude
	Mat clsmagI = clsplane[0];
multiply(clsmagI,clsmagI,clsmagI);
clsmagI *=0.05;
//cls-0--------------------
	Mat planes[2];
...
//<<factor;
factor = clsmagI.clone();///////////////////////////////////////////////////////////////////////////  overwriting the snr with cls
