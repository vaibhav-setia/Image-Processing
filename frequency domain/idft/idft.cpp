...
Mat updateMag(Mat complex )
{
    ...
    magnitude(planes[0], planes[1], magI);    // sqrt(Re(DFT(I))^2 + Im(DFT(I))^2)
       ...
       magI += Scalar::all(1);			 // switch to logarithmic scale: log(1 + magnitude)
    log(magI, magI);
 
    shift(magI);
    normalize(magI, magI, 1, 0, NORM_INF); // Transform the matrix with float values into a viewable image form (float between values 0 and 1).
             return magI;                                
